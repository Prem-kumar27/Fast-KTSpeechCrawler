...
class CaptionRegexMatcher(BaseFilter):
    ...
    def __call__(self, input):
        subtitles = input['subtitles']
        input['subtitles'] = list(filter(lambda s: re.match(
            self.regexp, s['original_phrase']) is not None, subtitles))
        return input
...
class CaptionNormalizer(BaseFilter):
    def __call__(self, input):
        for sub_info in input['subtitles']:
            sub_info['original_phrase'] = normalize_subtitle(
                sub_info["original_phrase"])
        return input
...
class CaptionLengthFilter(BaseFilter):
    ...
    def __init__(self, min_length=None, max_length=None):
        ...
        self.min_filter_func = lambda x: len(
            x.split()) >= min_length if min_length else lambda x: True
        self.max_filter_func = lambda x: len(
            x.split()) <= max_length if max_length else lambda x: True
...
class CaptionLeaveOnlyAlphaNumCharacters(BaseFilter):
    ...
    def __call__(self, input):
        for sub_info in input['subtitles']:
            sub_info['original_phrase'] = leave_alphanum_characters(
                sub_info["original_phrase"])
        return input
